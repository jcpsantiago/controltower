...
(def n-airports 
  (format "%,12d" (count (keys all-airports))))
...
(defn plane-svg
  ([path airline-name viz]
   [:div {:class (str "fl w-100 w-20-ns ph1 tc db-ns " viz)}
    [:div
      [:img {:src path}]
      [:p {:class "ma0 code f5"} airline-name]]])

  ([path airline-name]
   (plane-svg path airline-name "")))

(defn svg-icon
  ([url path viewbox]
   [:a {:href url
        :class "mh2 moon-gray hover-orange"}
    [:svg {:class "dib h2 w2"
           :fill "currentColor"
          ...
          [:div {:class "center"}
            ...
            [:div {:class "w-90 w-60-ns center mb4"}
              ...
              [:div {:class "f3-ns f4 lh-copy measure athelas"}
                [:p "We saw flights landing in nearby Tegel airport from our office, and wondered where all those airplanes came from. Sure, there is flightradar24.com, but that was too easy. So I made control tower, and now we ask Slack for answers."]
                [:p "To work the bot needs permissions for slash commands, and incoming hooks. We have a dedicated channel #planespotting for planespotting where each sighting is posted.
The control tower will reply privately if you call it anywhere else—this way you don't need to change channels."]
                [:img {:class "mv5-ns mv4"
                       :src "/img/screenshot.png"}]
                [:div {:class "cf"}
                  [:div {:class "fl w-70-ns w-100 pr3-ns"}
                   [:p "The control tower supports " n-airports " airports with scheduled flights in the world thanks to data from https://ourairports.com/. To look for flights use the command /spot followed by either the IATA code of the airport, the name of the city in english or random to look at a random airport. If the control tower doesn't see any flights in the air, you get back the current weather in that location.
"]]
            ...
            [:div {:class "mt5 mb3 pv5 avenir f4"
                   :style "background-color:#8FAADC"}
            ...
            [:footer {:class "athelas w-100 pv4 ph3 ph5-m ph6-l light-silver"}
             ...
             [:div {:class "tc mt3 center"}
              (svg-icon "https://github.com/jcpsantiago/controltower"
